...
### 9.2-b 로그 내용(content) 커버리지 갭

파이프라인 안정성(§9.2)과 별개로, **무엇을 남기는가** 관점의 갭. 이벤트 종류는 양쪽 다 넓게 남기나(피드백 라이프사이클·pdf·chat·rating·sync·CRUD / BE는 LLM tokens·cost·latency까지) 트리아지에 필요한 필드·이벤트가 비어 있다.

| # | 갭 | 영향 | 근거 |
|---|---|---|---|
| C1 | **FE 로그에 환경 컨텍스트 전무** — OS/앱 버전·빌드·디바이스 모델·로케일·네트워크 타입 없음 | "어느 iOS/버전/기기"인지 추적 불가 | `systemVersion/appVersion/UIDevice` 부재 |
| C2 | **FE↔BE 상관관계 ID 부재** — `request_id`가 피드백 플로우에만(`feedback.py:51,182`, 서버 `rid`), chat·pdf·sync·guide엔 없고 FE 로그도 서버 id 미반영 | 한 유저 액션을 클라+서버 로그로 stitching 불가 | 타 플로우 correlation id 없음 |
| C3 | **무음 실패 로그 없음** — `try? db.save…`(`NoteView.swift:488,507,551`, `FeedbackChatSheet.swift:231,296`) 실패 시 로그 0 | 데이터 유실이 흔적 없이 발생 | §5.2 지점, 로그 부재 |
| C4 | **BE 통일 요청/액세스 로그 없음** — method/path/status/latency/user 일관 미기록, 전역 예외 핸들러 부재로 미처리 5xx 컨텍스트 없이 누락 | 에러율·느린 요청 식별 구멍 | `main.py` CORS만 |
| C5 | **빌드/버전/커밋 SHA 미기록** — startup 로그에 storage 백엔드만, 앱 버전/git SHA 없음 | "어느 배포가 낸 로그"인지 불명 | `logging.py`/`main.py` startup |
| C6 | **업스트림 LLM/임베딩 에러 분류 없음** — Anthropic 429/529(overloaded)/timeout을 `log.exception`으로 뭉뚱그림, httpx WARNING 묵음 | 장애 원인(레이트리밋 vs 다운) 구분 불가 | `feedback_service`/`retrieval` except 일반화 |
| C7 | **감사(audit) 이벤트 부재** — 로그인 성공·로그아웃·계정 변경 미기록(JWT 실패만), `/api/admin/usage` 접근 무로깅 | 보안 사고·오남용 추적 불가 | `auth.py` 실패만, admin 접근 무로깅 |
| C8 | **(역방향) PII 과다 로깅** — 사용자 콘텐츠 평문 잔존: `Recognition (Haiku): '%s'`(text[:100]), 채팅 `content=%s`, query rewrite의 user_query | 개인정보 평문 로그 | `feedback_service`/chat/query rewrite 로그 |
| C9 | **클라 레벨·지연 부족** — `LogService` info/error만(warn/debug 없음), 클라 라운드트립 latency 대부분 미측정(pdf만 `ms`) | 심각도 구분·성능 추적 약함 | `LogService.swift`, appLog 호출 |

### 9.2-c 유즈케이스별 클라 로깅 커버리지

이벤트 종류는 넓지만(✅ 다수), **사용자 여정 단위**로 보면 인증/세션·일부 sync 경로가 비어 있다.

| 유즈케이스 | 커버리지 | 근거 / 갭 |
|---|---|---|
| 앱 시작·세션 복원 | ❌ | `AuthService.initialize` 무로깅 |
| 로그인(이메일)/회원가입 | ❌ | `signIn`/`signUp` 무로깅 |
| Google OAuth | ❌ | `signInWithGoogle` start/콜백/성공/실패 전부 무로깅 — 가장 디버깅 어려운 플로우인데 흔적 0 |
| 로그아웃 | ❌ | `signOut` 무로깅 |
| 노트 목록·CRUD | ✅ | `home` load/create/update/deleteNote |
| 노트 열기·드로잉 | ✅ | `note` loaded/loadDrawing/saveDrawing/newPage/goToPage |
| AI 피드백 요청 | ✅ 우수 | `note` feedback start→strokes→capture→received/failed |
| 피드백 카드 삭제 | ❌ 무음 | `NoteView.swift:642` `try? db.deleteFeedback` 로그 없음(실패도 무음) |
| 후속 채팅 / 평가 | ✅ | `chat`, `rating` 송신/실패 |
| PDF 업로드·뷰어·가이드 | ✅ | `pdf-upload`, `pdf` 전반 |
| 동기화 | ⚠️ 부분 | 실패·missing_blob·reachability만. 성공/볼륨(push n·pull n merged)/트리거 사유/`onLogin` full-pull/`onLogout`/claim/background flush 무로깅 |
| 설정 변경 | ❌ | `SettingsSheet` 언어·렌더모드·signOut 무로깅 |
| 앱 라이프사이클 | ❌ | `scenePhase` foreground/background 전환 무로깅(`ScatchLMApp.swift:33`) |

**핵심:** 🔴 **인증/세션 = 완전 사각지대** — 프로덕션 지원 1순위 문의(로그인 실패/예기치 않은 로그아웃/계정 전환 후 데이터 이슈)에 클라 로그가 0. 환경 컨텍스트(C1) 부재와 겹쳐 재현 외 추적 불가. 🟠 sync 성공/볼륨 비가시 → 유실 의심 신고 추적 불가. 🟠 파괴적 액션(카드 삭제) 무음.
...
| O9 | iOS | **FE 로그 컨텍스트(C1·C9)**: OS/앱버전/빌드/디바이스/로케일 + user_id·session_id 첨부, warn 레벨·라운드트립 latency 추가 | 🟠 관측 |
| O10 | 양쪽 | **상관관계 ID(C2)**: 서버 생성 request_id를 전 엔드포인트 응답 헤더로 내려 FE 로그에 동봉 → FE↔BE stitching | 🟠 관측 |
| O11 | iOS | **무음 실패 로깅(C3)**: `try?` DB 저장 실패 지점에 `appLogError` + (§5.2 롤백과 연계) | 🟠 데이터 |
| O12 | BE | **에러 분류·감사·PII(C6·C7·C8)**: 업스트림 429/529/timeout 구분 로깅, 로그인·admin 접근 audit, 사용자 콘텐츠 로그 마스킹/길이 제한 | 🟡 관측·보안 |
| O13 | BE | **빌드 식별(C5)**: startup 로그에 앱 버전·git SHA·env 기록 | 🟡 관측 |
| O14 | iOS | **인증/세션 로깅(§9.2-c, 🔴)**: 로그인·회원가입·OAuth(start/콜백/성공/실패)·로그아웃·세션 복원·만료 이벤트 로깅 (콘텐츠 PII 제외) | 🔴 관측 |
| O15 | iOS | **sync·라이프사이클 로깅**: sync 성공/볼륨·트리거 사유·onLogin/onLogout·background flush, scenePhase 전환, 카드 삭제 등 파괴적 액션 | 🟠 관측 |
