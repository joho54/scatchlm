...
> **bake는 결국 리버트했다.** bake(KaTeX→이미지)는 "동시 webview 16개 메모리"를 막으려 넣었는데,
> **그 메모리 크래시는 한 번도 확인된 적이 없다**(SE 16 webview ≈ 400MB ≪ jetsam 임계 1.3GB+;
> 잡힌 크래시는 전부 워치독/App Hang=키보드). 게다가 bake는 스냅샷 높이가 기기/콘텐츠마다 어긋나
> **하단 잘림**이 생기고(버퍼로도 whack-a-mole), 라이브 webview는 JS가 실제 높이를 보고 self-size해
> 잘림이 없다. → **bake 제거, 라이브 webview로 복원.** 진짜 수정(키보드)은 유지.

- `Views/BakedMarkdownView.swift` — **라이브 self-sizing WKWebView**(JS `heightChanged`로 높이
  측정 → 프레임 매칭). 채팅 버블은 `preferBake`로 전부 webview 렌더(MarkdownUI 중첩 ForEach 행도
  원천 제거). `KaTeXSnapshotRenderer`·`injectBakeWidth`·template `__WIDTH__`/katexReady는 제거.
