...
/// 서재 기반 "새로 도전해볼 분야" 제안(Sonnet)의 세션 캐시 + single-flight 로더.
///
/// 홈 프롬프트 바와 discover 시트가 같은 결과를 공유하게 한다 — 홈은 자주 그려지므로
/// 매 표시마다 LLM을 부르면 낭비다. 첫 호출만 네트워크를 타고, 이후엔 캐시를 돌려준다.
...
actor DiscoverSuggestionLoader {
    ...
    private var cached: [DiscoverSuggestion]?
    private var inflight: Task<[DiscoverSuggestion], Never>?

    private init() {}

    /// 캐시가 있으면 즉시, 없으면 1회만 네트워크(동시 호출은 같은 Task를 공유). 실패는 빈 배열.
    func load(language: String) async -> [DiscoverSuggestion] {
        if let cached { return cached }
        if let inflight { return await inflight.value }
        let task = Task { () -> [DiscoverSuggestion] in
