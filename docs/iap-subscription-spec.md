...
| # | 레버 | 적용 지점 | 효과 | 난이도 | 출시 전? |
|---|---|---|---|---|---|
| L1 | **output max_tokens 캡** | `feedback_service.py:160` (`max_tokens=4096`) | 긴 응답/워스트케이스 output 비용 상한 | 낮음 | ✅ 권장 |
| L2 | **프롬프트 캐싱** | `system`/`messages`에 `cache_control` | input의 캐시 적중분 **0.1×**. chat·동일교재 연속에 강함 | 중간 | △ chat 우선 |
| L3 | **모델 라우팅 강화** | `_select_model` (현 task_type 의존) | Haiku 라우팅분 건당 **~20×↓**($0.02→$0.001) | 중간 | fast-follow |
| L4 | **이미지 토큰 절감** | 업로드 전 해상도/압축 (iOS) + 서버 | Vision input 토큰 직접 절감 | 낮음 | fast-follow |
...
**L3 — 모델 라우팅 강화.**
현 `_select_model`은 클라이언트가 보내는 `task_type`(기본 `complex`)에만 의존 → 사실상 전부 Sonnet 가능성. 서버측 휴리스틱으로 Haiku 비중↑:
- 이미 도는 **인식 Haiku 사전패스**(`get_recognition`) 결과로 난이도 추정(짧은 손글씨·교재 컨텍스트 없음 → Haiku 피드백).
- 또는 Haiku 트리아지 1콜로 "단순 첨삭 vs 개념 설명" 분기. 라우팅분은 건당 ~20× 절감이라 **분포만 옮겨도 평균 단가 급락**.

**L4 — 이미지 토큰 절감.**
Vision input 토큰은 이미지 픽셀 수에 비례. iOS 업로드 전 **장변 해상도 캡 + 압축**(`get_feedback`이 받는 `image_bytes` 자체를 작게)으로 input 직접 절감. 인식·피드백 둘 다 이미지를 보내므로 효과 2배 지점.
...
- **남음(fast-follow):** L3 라우팅, L4 이미지, 그리고 **캐시 적중률 측정용 `LLMUsage` 캐시 토큰 컬럼**(§11.4) 추가. 피드백 one-shot 캐싱은 효과 제한적이라 미적용(§11.3).
