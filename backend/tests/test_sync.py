...
def _chat(id_: str, updated_at: str, session_id: str, feedback_id: str | None = None,
          content: str = "안녕", role: str = "user", deleted: bool = False,
...
@pytest.mark.asyncio
async def test_chat_message_quote_preserved(client: AsyncClient, auth_header: dict):
    """chat_message.quote('선택 질문' 인용 구절)가 push→pull로 보존되어야 한다.

    라이브 모드 인용 칩이 시트 재진입·cross-device에서 유실되던 버그의 회귀 가드.
    """
    session = _session("sess-q", "2026-06-06T02:00:00Z")
    msg = _chat("msg-q", "2026-06-06T02:00:01Z", session_id="sess-q",
                quote="τὸ γὰρ αὐτὸ νοεῖν ἐστίν τε καὶ εἶναι")
    res = await client.post(
        "/api/sync/push", headers=auth_header,
        json={"changes": {**_empty_changes(), "sessions": [session], "chat_messages": [msg]}},
    )
    ...
    msgs = pull.json()["changes"]["chat_messages"]
    assert len(msgs) == 1
    assert msgs[0]["quote"] == "τὸ γὰρ αὐτὸ νοεῖν ἐστίν τε καὶ εἶναι"


@pytest.mark.asyncio
async def test_chat_message_quote_omitted_defaults_null(client: AsyncClient, auth_header: dict):
    """quote 키를 보내지 않는 구버전 클라이언트도 안전(skip)하고, pull은 quote=null을 반환한다."""
    session = _session("sess-noq", "2026-06-06T02:00:00Z")
    msg = _chat("msg-noq", "2026-06-06T02:00:01Z", session_id="sess-noq")
    del msg["quote"]  # 구버전 클라이언트 시뮬레이션
    res = await client.post(
        "/api/sync/push", headers=auth_header,
        json={"changes": {**_empty_changes(), "sessions": [session], "chat_messages": [msg]}},
    )
