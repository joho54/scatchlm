...
async def test_ensure_refreshes_on_template_change(db_session, monkeypatch):
    """템플릿 PDF가 교체되면(해시 변경) 기존 사본을 자동 갱신한다(페이지/챕터 포함)."""
    ...
    # 템플릿이 3쪽짜리 새 PDF로 바뀐 상황을 시뮬레이트(_template 캐시 교체).
    import fitz
    doc = fitz.open()
    for _ in range(3):
        doc.new_page()
    ...
    assert tb1.content_hash == new_hash != old_hash
    assert tb1.total_pages == 3
    chs = (await db_session.execute(select(Chapter).where(Chapter.textbook_id == tid))).scalars().all()
    assert len(chs) == 1 and chs[0].page_end == 3   # 챕터 재생성 + 페이지 반영
