...
- [x] **§A.5 복구 드릴 1회 수행** — 2026-06-04 로컬 격리 컨테이너에 `scatchlm-2026-06-04.dump` 복원. 15테이블 row 수 prod 완전 일치, pgvector(vector(512)) 정상, ~0.15s. 절차+기록 DEPLOY.md
- [x] §A.3 Object Storage lifecycle 보존 — 2026-06-04 콘솔에서 `backups/db/` **60일 만료** 설정. NCP가 S3 lifecycle API를 노출 안 해 **CLI 검증 불가**(DEPLOY.md NCP 특이점). 미작동해도 실패 모드는 양성(덤프 누적). 주간 티어는 prefix 분리 필요 → 후속
