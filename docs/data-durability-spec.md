...
**Part A — 백업 (실질 최우선)**
- [ ] **§A.3 pg_dump → Object Storage 일배치 cron** (스트림 업로드 + 임시파일 삭제)
- [ ] **§A.5 복구 드릴 1회 수행 + 절차 문서화** (pgvector 확장 검증 포함)
- [ ] §A.3 Object Storage lifecycle 보존(일 14 / 주 8 초안) 설정
- [ ] §A.6 `.env.prod` 시크릿 매니저 보관 + 복구 전제 문서화
- [ ] §A.4.1 (선택) NCP VM 스냅샷 주기 설정
- [ ] 백업 성공/실패 알림(실패가 조용하면 백업 부재와 동일 — 텔레메트리 사각지대 교훈)

**Part B — 로그 적재**
- [ ] `backend/app/models/app_log.py` — `AppLog` 모델 (§B.3)
- [ ] `app/models/__init__.py` / alembic `env.py` import 등록 (메모리 `project_alembic_baseline`)
- [ ] `alembic revision --autogenerate -m "add app_logs table"` + `upgrade head`
- [ ] `devlog.py` — 배치 핸들러 best-effort DB 적재 (§B.4 Track A)
- [ ] 회귀 테스트: 적재 성공 / DB 실패 시 정상 반환 / ts 폴백 (작성만, 실행 CI — 메모리 `feedback_skip_test_execution`)
- [ ] 운영 마이그레이션 수동 적용
- [ ] (선택) Track C 로테이션 / Track D retention 합의
- [ ] **(필수 후속) `.claude/commands/check-prod-logs.md` `users`/`stats`를 SQL 집계로 전환** (§B.4 Track B)
