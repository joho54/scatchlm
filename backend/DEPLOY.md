...
- `ensure_demo_textbook`은 **생성 시점 동결(create-once)** — 사본이 없으면 만들고, **이미 있으면
  갱신 안 함**(클라 PDF 캐시와 항상 동일본 → 데모 중 불일치 방지). 교체는 **신규 유저에게만** 반영.
- 기존 유저에게 강제 반영하려면 그 유저의 `demo-{user_id}` row + `{user_id}_demo.pdf`를 지운다.
- 페이지 수는 생성 시점 PDF에서 자동 추출. iOS 표시용 상수 `OnboardingView.demoTextbookPages`만
  페이지 수에 맞추면 된다. 자세한 절차: `backend/app/assets/README.md`.
