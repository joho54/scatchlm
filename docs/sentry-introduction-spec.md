...
- **Track A (BE): 완료.** `sentry-sdk[fastapi]==2.42.1` 핀. `app/core/sentry.py`(init + `before_send` PII 스크럽), `main.py`에서 `app` 생성 전 `init_sentry()`. 미들웨어(`request_log.py`)가 incoming `sentry-trace` 파싱→`trace_id` contextvar+Sentry 태그, 미처리 예외 `capture_exception()`. 모든 로그 포맷 `[trace:… req:…]`로 확장(`logging.py`/`request_context.py`), devlog가 FE trace_id 출력. `SENTRY_DSN`/`SENTRY_TRACES_SAMPLE_RATE` env. **테스트 73 passed** + 신규 `tests/unit/test_sentry.py`(스크럽/파싱 regression 4건).
- **Track B (iOS): 완료·clean 빌드 검증(BUILD SUCCEEDED), 벤더 XCFramework로 배포 가능.** `Observability.swift`(start/scrub/currentTraceId/setUser/captureServerError), `ScatchLMApp.init()`에서 `Observability.start()`, `tracePropagationTargets`를 백엔드 호스트로 한정, `LogService.context()`에 trace_id, APIClient 5xx 캡처, AuthService `setUser`. `Config.sentryDSN`.
  - **통합 방식(커밋 상태): 로컬 XCFramework 벤더링.** `Vendor/Sentry.xcframework`(sentry-cocoa 8.58.3 static, iOS device+simulator 슬라이스만 ~49MB, 커밋됨) + `project.yml`에서 `framework: Vendor/Sentry.xcframework`(embed:false) + `OTHER_LDFLAGS:-lc++`. **`clean build` BUILD SUCCEEDED**(`ProcessXCFramework` 링크 확인, undefined symbol 0). archive도 SPM 다운로드 없이 이 방식으로 통과 → **iOS 출시 빌드 가능.** 검증 중 버그 1건 수정: `SentryRequest`엔 `data` 필드 없음 → 스크럽에서 제거.
  - **왜 SPM이 아니라 벤더링인가:** 정식 통합은 sentry-cocoa SPM이나, `xcodebuild`가 SPM으로 XCFramework(53MB) 다운로드(`BinaryArtifactsManager.download`)에서 **데드락**(0바이트, curl로는 3.9초) → archive 불가. SwiftPM/xcodebuild 환경 버그(코드 결함 아님). 벤더링은 동일 SDK 바이너리를 다운로드 없이 링크 → 결정적. 환경 이슈 해소(또는 CI에서 다운로드 가능) 시 `project.yml`의 주석 처리된 SPM 블록으로 복구 가능. 진단은 메모리 `project_sentry_spm_deadlock` 참고.
- **Track C: C-2(env/문서) 완료.** `.env.prod.example` Sentry 변수 + `backend/DEPLOY.md`에 C-1(프로젝트 생성)·C-2(DSN 주입)·C-3(dSYM) 수동 절차. C-1/C-3은 sentry.io 대시보드·릴리스 빌드 필요(수동).
